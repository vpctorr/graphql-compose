...
describe('ObjectTypeComposer', () => {
  ...
  describe('fields manipulation', () => {
    ...
    describe('setFields()', () => {
      ...
      it('should add fields with converting types from string to object', () => {
        ...
        expect((tc.getFieldType('field4') as any).ofType).toBe(GraphQLInt);
        expect(tc.getFieldType('field5')).toBeInstanceOf(GraphQLNonNull);
        expect((tc.getFieldType('field5') as any).ofType).toBe(GraphQLBoolean);
      });
      ...
      it('should add fields with converting args types from string to object', () => {
        ...
        expect((tc.getFieldArgType('field3', 'arg1') as any).ofType).toBe(GraphQLString);
        expect(tc.getFieldArgType('field3', 'arg2')).toBeInstanceOf(GraphQLList);
        expect((tc.getFieldArgType('field3', 'arg2') as any).ofType).toBe(GraphQLFloat);
      ...
      it('accept types as function', () => {
        const typeAsFn = (s: SchemaComposer<any>) => {
          expect(s).toBeInstanceOf(SchemaComposer);
        ...
        if (graphqlVersion >= 14) {
          expect((tc.getType() as any)._fields().input3.type).toBe(GraphQLString);
        } else {
          expect((tc.getType() as any)._typeConfig.fields().input3.type).toBe(GraphQLString);
        }
      });
    ...
    it('addNestedFields()', () => {
      ...
      if (fieldTC instanceof ObjectTypeComposer) {
        ...
        expect((tc.getFieldType('fieldNested2') as any).ofType).toBe(GraphQLInt);
      }
    });
    ...
    describe('check modificators', () => {
      ...
      it('makeFieldNonNull()', () => {
        ...
        expect((tc.getFieldType('fieldNN') as any).ofType).toBe(GraphQLString);
  ...
  describe('interfaces manipulation', () => {
    ...
    it('addInterface()', () => {
      ...
      tc.addInterface((s: SchemaComposer<any>) => {
        expect(s).toBeInstanceOf(SchemaComposer);
        return iftc;
      });
  ...
  describe('create() [static method]', () => {
    ...
    it('should create TC by SDL', () => {
      ...
      expect((myTC.getFieldType('f2') as any).ofType).toBe(GraphQLInt);
    });
  ...
  describe('cloneTo()', () => {
    ...
    it('interfaces should be different', () => {
      ...
      const ifaceA = tc.getInterfaces()[0] as InterfaceTypeComposer<any, any>;
      const ifaceACloned = cloned.getInterfaces()[0] as InterfaceTypeComposer<any, any>;
    ...
    it('resolvers should be cloned with all subtypes', () => {
      ...
      expect((findMany.type as any).ofType).not.toBe((findManyCloned.type as any).ofType);
  ...
  describe('get()', () => {
    it('should return type by path', () => {
      ...
      expect((myTC.get('field1') as any).getType()).toBe(GraphQLString);
      expect((myTC.get('field1.@arg1') as any).getType()).toBe(GraphQLInt);
    });
  });
  ...
  describe('Resolvers manipulation', () => {
    ...
    it('addResolver() should add stub resolve method', () => {
      ...
      tc.addResolver(ResolverDefinition);
      expect((tc.getResolver('myResolver3') as any).resolve(undefined)).toEqual({});
    });
  ...
  describe('addRelation()', () => {
    ...
    describe('_relationWithResolverToFC()', () => {
      it('should return FieldConfig', () => {
        const fc = (ArticleTC as any)._relationWithResolverToFC({
    ...
    describe('thunk with Resolver', () => {
      it('should convert simple relation to fieldConfig', () => {
        ...
        const fc = ArticleTC.getType().getFields().user as any;
      ...
      it('should convert cross related relations to fieldConfigs', () => {
        ...
        const fc1 = ArticleTC.getType().getFields().user as any;
        expect(fc1.type.name).toBe('User');

        const fc2 = UserTC.getType().getFields().lastArticle as any;
  ...
  it('should have chain-methods', () => {
    ...
    expect(tc.addRelation('user', {} as any)).toBe(tc);
  ...
  describe('merge()', () => {
    ...
    it('should throw error on wrong type', () => {
      ...
      expect(() => otc.merge(schemaComposer.createScalarTC('Scalar') as any)).toThrow(
