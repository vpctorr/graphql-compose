...
describe('TypeMapper', () => {
  ...
  it('should create input object type from template string', () => {
    ...
    expect((IntRangeTC.getFieldType('max') as GraphQLNonNull<any>).ofType).toBe(GraphQLInt);
  });
  ...
  it('should return wrapped type', () => {
    ...
    expect(typeMapper.convertSDLWrappedTypeName('[String]!')).toBeInstanceOf(NonNullComposer);
    expect(
      (typeMapper.convertSDLWrappedTypeName('[String]!') as NonNullComposer<any>).ofType
    ).toBeInstanceOf(ListComposer);
  ...
  describe('convertOutputFieldConfig()', () => {
    describe('converting field type', () => {
      ...
      it('should accept GraphQLNonNull', () => {
        ...
        expect((fc.type.getType() as GraphQLNonNull<any>).ofType).toBe(GraphQLString);
      });
      ...
      it('should accept GraphQLList', () => {
        ...
        expect((fc.type.getType() as GraphQLList<any>).ofType).toBe(GraphQLString);
      });
      ...
      it('should lookup type name as string in schemaComposer', () => {
        ...
        const fc2 = typeMapper.convertOutputFieldConfig({ type: '[MyType]' });
        expect(fc2.type).toBeInstanceOf(ListComposer);
        expect((fc2.type.getType() as GraphQLList<any>).ofType).toBe(tc.getType());
      });
      ...
      it('should create field config from GraphQL Schema Language', () => {
        ...
        const tc = fc.type as ObjectTypeComposer;
      ...
      it('should create field with Enum type from GraphQL Schema Language', () => {
        ...
        const enumValues = (fc.type as EnumTypeComposer).getFields();
      ...
      it('should accept Resolver', () => {
        ...
        expect((fc as any).resolve()).toBe(123);
      });
      ...
      it('should accept array with one element as type and wrap them with GraphQLList', () => {
        const fc = typeMapper.convertOutputFieldConfig(['String']) as any;
        expect(fc.type).toBeInstanceOf(ListComposer);
        expect(fc.type.ofType).toBeInstanceOf(ScalarTypeComposer);
        expect(fc.type.ofType.getType()).toBe(GraphQLString);

        const fc2 = typeMapper.convertOutputFieldConfig({ type: ['String'] }) as any;
        expect(fc2.type).toBeInstanceOf(ListComposer);
        ...
        const fc4 = typeMapper.convertOutputFieldConfig([tc]) as any;
        expect(fc4.type).toBeInstanceOf(ListComposer);
        expect(fc4.type.getType().ofType).toBe(tc.getType());

        const fc5 = typeMapper.convertOutputFieldConfig({ type: [tc] }) as any;
  ...
  describe('convertInputFieldConfig()', () => {
    ...
    it('should accept GraphQLNonNull', () => {
      const ic = typeMapper.convertInputFieldConfig(new GraphQLNonNull(GraphQLString)) as any;
      expect(ic.type).toBeInstanceOf(NonNullComposer);
      expect(ic.type.getType().ofType).toBe(GraphQLString);
    });

    it('should accept GraphQLList', () => {
      const ic = typeMapper.convertInputFieldConfig(new GraphQLList(GraphQLString)) as any;
    ...
    it('should create field with Enum type from GraphQL Schema Language', () => {
      const fc = typeMapper.convertInputFieldConfig('enum MyInputEnum { AND OR }') as any;
      expect(fc.type).toBeInstanceOf(EnumTypeComposer);
    ...
    it('should pass unchanged thunk', () => {
      ...
      const tc = typeMapper.convertInputFieldConfig(myTypeThunk) as any;
    ...
    it('should accept array with one element as type and wrap them with GraphQLList', () => {
      const fc = typeMapper.convertInputFieldConfig(['String']) as any;
      expect(fc.type).toBeInstanceOf(ListComposer);
      expect(fc.type.getType().ofType).toBe(GraphQLString);

      const fc2 = typeMapper.convertInputFieldConfig({ type: ['String'] }) as any;
      expect(fc2.type).toBeInstanceOf(ListComposer);
      expect(fc2.type.getType().ofType).toBe(GraphQLString);
      ...
      const fc4 = typeMapper.convertInputFieldConfig([itc]) as any;
      expect(fc4.type).toBeInstanceOf(ListComposer);
      expect(fc4.type.getType().ofType).toBe(itc.getType());

      const fc5 = typeMapper.convertInputFieldConfig({ type: [itc] }) as any;
      expect(fc5.type).toBeInstanceOf(ListComposer);
      expect(fc5.type.getType().ofType).toBe(itc.getType());

      const fc6 = typeMapper.convertInputFieldConfig([['String']]) as any;
      expect(fc6.type).toBeInstanceOf(ListComposer);
  ...
  describe('convertArgConfig()', () => {
    ...
    it('should accept GraphQLNonNull', () => {
      const ac = typeMapper.convertArgConfig(new GraphQLNonNull(GraphQLString)) as any;
      expect(ac.type).toBeInstanceOf(NonNullComposer);
      expect(ac.type.getType().ofType).toBe(GraphQLString);
    });

    it('should accept GraphQLList', () => {
      const ac = typeMapper.convertArgConfig(new GraphQLList(GraphQLString)) as any;
    ...
    it('should create arg config with Enum type from GraphQL Schema Language', () => {
      const fc = typeMapper.convertArgConfig('enum MyArgEnum { AND OR }') as any;
      expect(fc.type).toBeInstanceOf(EnumTypeComposer);
    ...
    it('should accept array with one element as type and wrap them with GraphQLList', () => {
      const fc = typeMapper.convertArgConfig(['String']) as any;
      expect(fc.type).toBeInstanceOf(ListComposer);
      expect(fc.type.getType()).toBeInstanceOf(GraphQLList);
      expect(fc.type.getType().ofType).toBe(GraphQLString);

      const fc2 = typeMapper.convertArgConfig({ type: ['String'] }) as any;
      expect(fc2.type).toBeInstanceOf(ListComposer);
      expect(fc2.type.getType()).toBeInstanceOf(GraphQLList);
      expect(fc2.type.getType().ofType).toBe(GraphQLString);

      const fc3 = typeMapper.convertArgConfig({ type: [GraphQLString] }) as any;
      expect(fc3.type).toBeInstanceOf(ListComposer);
      ...
      const fc4 = typeMapper.convertArgConfig([itc]) as any;
      expect(fc4.type).toBeInstanceOf(ListComposer);
      expect(fc4.type.getType()).toBeInstanceOf(GraphQLList);
      expect(fc4.type.getType().ofType).toBe(itc.getType());

      const fc5 = typeMapper.convertArgConfig({ type: [itc] }) as any;
      expect(fc5.type).toBeInstanceOf(ListComposer);
      expect(fc5.type.getType()).toBeInstanceOf(GraphQLList);
      expect(fc5.type.getType().ofType).toBe(itc.getType());

      const fc6 = typeMapper.convertArgConfig([['String']]) as any;
      expect(fc6.type).toBeInstanceOf(ListComposer);
  ...
  describe('parseTypesFrom... methods', () => {
    it('parseTypesFromString()', () => {
      ...
      const ts = typeMapper.parseTypesFromString(gql) as any;
    ...
    it('parseTypesFromString() should accept `union` definition', () => {
      ...
      const TypeAB = ts.get('TypeAB') as UnionTypeComposer;
  ...
  describe('convertSDLTypeDefinition()', () => {
    it('should return same type for the same TypeDefinitionString', () => {
      const t1 = typeMapper.convertSDLTypeDefinition(
        'type SameType { a: Int }'
      ) as ObjectTypeComposer;
      const t2 = typeMapper.convertSDLTypeDefinition(
        'type SameType { a: Int }'
      ) as ObjectTypeComposer;
      expect(t1).toBe(t2);
    ...
    it('ObjectType', () => {
      const tc = typeMapper.convertSDLTypeDefinition(`
        type My1 @typeDirective(a: false) { 
          a: Int @cost(value: 1)
          b: Float @inexistent(q: 1, w: true, e: "s")
          c: String @me @they @me
