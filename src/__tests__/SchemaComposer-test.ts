...
describe('SchemaComposer', () => {
  ...
  it('should implements `has` method`', () => {
    ...
    expect(sc.has('inexistentType')).toBe(false);
  });

  describe('constructor', () => {
    describe('if GraphQLSchema provided', () => {
      const gqUser: GraphQLObjectType = new GraphQLObjectType({
        name: 'User',
        fields: () => ({
      ...
      it('should merge nonstandard root types', () => {
        const sc = new SchemaComposer(gqSchema);
        expect(sc.Query.getFieldNames()).toEqual(['post', 'user']);
        expect(sc.has('OopsQuery')).toBeTruthy();
        expect(sc.Query).toBe(sc.get('OopsQuery'));
      });
    });
  ...
  describe('buildSchema()', () => {
    it('should accept additional types', () => {
      ...
      expect((schema as any)._typeMap.Me1).toEqual(me1);
      expect((schema as any)._typeMap.Me2).toEqual(me2);
    });
  ...
  describe('removeEmptyTypes()', () => {
    ...
    it('should remove empty ROOT types: Mutation, Subscription', () => {
      const sc = new SchemaComposer(` 
        ...
        type Subscription
      `);
      const schema = sc.buildSchema() as any;
  ...
  describe('SchemaMustHaveType', () => {
    ...
    expect((sc as any)._schemaMustHaveTypes).toContain(tc);

    sc.clear();
    expect((sc as any)._schemaMustHaveTypes).not.toContain(tc);

    sc.addSchemaMustHaveType(tc);
    sc.Query.addFields({ time: 'String' });
    const schema = sc.buildSchema();
    expect((schema as any)._typeMap.Me).toEqual(tc.getType());
  });
  ...
  describe('works with directives', () => {
    ...
    function removeDefaultDirectives(sc: SchemaComposer<any>) {
  ...
  describe('createTC()', () => {
    it('should Create ObjectTC', () => {
      ...
      // should return the same type composer instance
  ...
  describe('clone()', () => {
    ...
    it('should clone MustHaveTypes', () => {
      ...
      const sc2 = sc1.clone();
      expect((sc2 as any)._schemaMustHaveTypes).toHaveLength(1);
      expect((sc2 as any)._schemaMustHaveTypes[0]).not.toBe((sc1 as any)._schemaMustHaveTypes[0]);
    });
