...
describe('InterfaceTypeComposer', () => {
  ...
  describe('fields manipulation', () => {
    ...
    describe('setFields()', () => {
      ...
      it('should add fields with converting types from string to object', () => {
        ...
        expect((iftc.getFieldType('field4') as any).ofType).toBe(GraphQLInt);
        expect(iftc.getFieldType('field5')).toBeInstanceOf(GraphQLNonNull);
        expect((iftc.getFieldType('field5') as any).ofType).toBe(GraphQLBoolean);
      });
      ...
      it('should add fields with converting args types from string to object', () => {
        ...
        expect((iftc.getFieldArgType('field3', 'arg1') as any).ofType).toBe(GraphQLString);
        expect(iftc.getFieldArgType('field3', 'arg2')).toBeInstanceOf(GraphQLList);
        expect((iftc.getFieldArgType('field3', 'arg2') as any).ofType).toBe(GraphQLFloat);
      ...
      it('accept types as function', () => {
        ...
        if (graphqlVersion >= 14) {
          expect((iftc.getType() as any)._fields().field6.type).toBe(GraphQLString);
        } else {
          expect((iftc.getType() as any)._typeConfig.fields().field6.type).toBe(GraphQLString);
        }
      });
      ...
      it('accept fieldConfig as function', () => {
        ...
        // show provide unwrapped/unhoisted type for graphql
        if (graphqlVersion >= 14) {
          expect((iftc.getType() as any)._fields().input4.type).toBe(GraphQLString);
        } else {
          expect((iftc.getType() as any)._typeConfig.fields().input4.type).toBe(GraphQLString);
        }
      });
    });
    ...
    it('makeFieldNonNull()', () => {
      ...
      expect((iftc.getFieldType('fieldNN') as any).ofType).toBe(GraphQLString);
  ...
  describe('interfaces manipulation', () => {
    ...
    it('addInterface()', () => {
      ...
      iftc.addInterface((s: SchemaComposer<any>) => {
  ...
  describe('create() [static method]', () => {
    ...
    it('should create Interface by type template string', () => {
      ...
      expect((myIFTC.getFieldType('f2') as any).ofType).toBe(GraphQLInt);
    });
  ...
  describe('get()', () => {
    it('should return type by path', () => {
      ...
      expect((myIFTC.get('field1') as any).getType()).toBe(GraphQLString);
      expect((myIFTC.get('field1.@arg1') as any).getType()).toBe(GraphQLInt);
    });
  });
  ...
  describe('typeResolvers methods', () => {
    ...
    let KindRedTC: ObjectTypeComposer;
    let KindBlueTC: ObjectTypeComposer;
    ...
    it('getTypeResolverCheckFn()', () => {
      const checkFn = iftc.getTypeResolverCheckFn(PersonTC) as any;
      expect(checkFn({ age: 15 })).toBeTruthy();
      expect(checkFn({ nope: 'other type' })).toBeFalsy();
    });

    it('setTypeResolverFallback()', () => {
      const checkFn1 = iftc.getResolveType() as any;
      ...
      const checkFn2 = iftc.getResolveType() as any;
  ...
  describe('directive methods', () => {
    ...
    it('should create directives via config as object', () => {
      const tc2 = schemaComposer.createInterfaceTC({
        name: 'MyObject',
        fields: {
      ...
      expect(tc2.toSDL()).toEqual(dedent`
        interface MyObject @ok(a: 1, b: "123", c: true) @go {
  ...
  describe('merge()', () => {
    ...
    it('should throw error on wrong type', () => {
      const iface = schemaComposer.createInterfaceTC(`interface IFace { name: String }`);
      expect(() => iface.merge(schemaComposer.createScalarTC('Scalar') as any)).toThrow(
