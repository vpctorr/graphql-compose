...
describe('UnionTypeComposer', () => {
  ...
  describe('create() [static method]', () => {
    ...
    it('should create UTC by UnionTypeConfig with inexistent types', () => {
      const myUTC = UnionTypeComposer.create(
        {
          name: 'TestType',
      ...
      const types = myUTC.getTypes() as any;
      expect(types).toHaveLength(2);
  ...
  describe('typeResolvers methods', () => {
    ...
    it('setTypeResolverFallback()', () => {
      ...
      const checkFn2 = utc.getResolveType() as any;
  ...
  describe('merge()', () => {
    ...
    it('should throw error on wrong type', () => {
      const resultUTC = schemaComposer.createUnionTC(`union Result = Article | Comment`);
      expect(() => resultUTC.merge(schemaComposer.createScalarTC('Scalar') as any)).toThrow(
